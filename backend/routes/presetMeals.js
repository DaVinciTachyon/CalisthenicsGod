...
router.post('/ingredients', async (req, res) => {
	...
	for (let i = 0; i < meal.ingredients.length; i++) {
		const ingredient = await Ingredients.findById(meal.ingredients[i].ingredientId);
		ingredients.push({
			_id: ingredient._id,
...
router.post('/removeIngredient', async (req, res) => {
	...
	const index = meal.ingredients.findIndex((val) => val.ingredientId == req.body.ingredientId);
	if (index === -1) return res.status(400).send({ error: 'Invalid Ingredient ID' });
